    ...
    <li className={`flex items-center py-2 px-4 justify-between gap-10 ${ptSans.className} text-lg hover:bg-pink-100`}>
      <div className='flex gap-10'>
        <span>
          ...
          <span className='ml-2'>{`${date.toLocaleString('default', { day: '2-digit' })}`}</span>
        </span>
      ...
      <div className='flex items-center gap-20'>
        ...
        <span className='flex gap-4'>
          {links.rsvp && (
            <a className='cursor-pointer font-bold transition-colors duration-200 hover:text-pink-500'>RSVP</a>
          )}

          {links.ticket && (
            <a className='cursor-pointer font-bold transition-colors duration-200 hover:text-pink-500'>Ticket</a>
          )}
        </span>
      </div>
    </li>
